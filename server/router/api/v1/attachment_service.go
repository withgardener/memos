...
func SaveAttachmentBlob(ctx context.Context, profile *profile.Profile, stores *store.Store, create *store.Attachment) error {
	...
	if instanceStorageSetting.StorageType == storepb.InstanceStorageSetting_LOCAL {
		...
		key, err := s3Client.UploadObject(ctx, filepathTemplate, create.Type, create.Filename, "public, max-age=31536000", bytes.NewReader(create.Blob))
		if err != nil {
			return errors.Wrap(err, "Failed to upload via s3 client")
		}
		presignURL, err := s3Client.PresignGetObject(ctx, key, create.Filename, "public, max-age=31536000")
		if err != nil {
